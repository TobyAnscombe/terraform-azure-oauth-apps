#!/usr/bin/env bash
# test-snowflake-oauth.sh
#
# Tests that a Snowflake External OAuth client app can obtain a valid token
# from Azure AD. Credentials are retrieved from CyberArk AAM/CP, or passed
# directly via --client-id / --client-secret for quick ad-hoc testing.
#
# Usage (CyberArk retrieval):
#   ./test-snowflake-oauth.sh \
#     --cyberark-cp-url https://cyberark-cp.example.com \
#     --cyberark-app-id snaplogic-groundplex-sandpit \
...
[[ -z "$TENANT_ID" || -z "$IDENTIFIER_URI" || -z "$APP_NAME" ]] && {
  echo "Usage: $0 --tenant-id ID --identifier-uri URI --app-name NAME [--env ENV]"
  echo "       (--cyberark-cp-url URL --cyberark-app-id ID --safe NAME) OR (--client-id ID --client-secret SECRET)"
  exit 1
}

TOKEN_URL="https://login.microsoftonline.com/${TENANT_ID}/oauth2/v2.0/token"
SCOPE="${IDENTIFIER_URI}/.default"
ACCOUNT_NAME="${APP_NAME}-${ENV}"

echo "--- Snowflake External OAuth test ---"
...
if [[ -n "$CLIENT_ID" && -n "$CLIENT_SECRET" ]]; then
  ...
  # Look up client_id from Azure AD app registration
  ...
  # Retrieve secret from CyberArk CP REST API
...
print(json.dumps({k: d[k] for k in ['aud','iss','appid','roles','exp'] if k in d}, indent=2))
" "$PAYLOAD")
...
[[ "$AUD" == "$IDENTIFIER_URI" ]] || {
  echo "[FAIL] Unexpected audience: got '${AUD}', expected '${IDENTIFIER_URI}'"
  exit 1
}
[[ "$APPID" == "$CLIENT_ID" ]] || {
  echo "[FAIL] Unexpected appid: got '${APPID}', expected '${CLIENT_ID}'"
  exit 1
}

echo ""
echo "[PASS] Snowflake External OAuth authentication verified"
echo "       Token is valid for Snowflake SECURITY INTEGRATION with audience ${AUD}"

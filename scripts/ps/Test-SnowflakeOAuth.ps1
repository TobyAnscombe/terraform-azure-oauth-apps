<#
.SYNOPSIS
    Tests that a Snowflake External OAuth client app can obtain a valid token from Azure AD.
...
.PARAMETER IdentifierUri
    App ID URI of the Snowflake resource server (e.g. api://svc-snowflake-oauth-resource-sandpit).
...
.EXAMPLE
    .\Test-SnowflakeOAuth.ps1 `
        -CyberArkCpUrl https://cyberark-cp.example.com `
        -CyberArkAppId snaplogic-groundplex-sandpit `
...
if (-not $TenantId -or -not $IdentifierUri -or -not $AppName) {
    Write-Host "Usage: .\Test-SnowflakeOAuth.ps1 -TenantId ID -IdentifierUri URI -AppName NAME [-Env ENV]"
    Write-Host "       (-CyberArkCpUrl URL -CyberArkAppId ID [-Safe NAME]) OR (-ClientId ID -ClientSecret SECRET)"
    exit 1
}

$TokenUrl    = "https://login.microsoftonline.com/$TenantId/oauth2/v2.0/token"
$Scope       = "$IdentifierUri/.default"
$AccountName = "$AppName-$Env"

Write-Host "--- Snowflake External OAuth test ---"
...
$Claims = Get-JwtClaims -Token $TokenResponse.access_token -Fields 'aud', 'iss', 'appid', 'roles', 'exp'
$Claims | ConvertTo-Json | Write-Host

if ($Claims['aud'] -ne $IdentifierUri) {
    Write-Host "[FAIL] Unexpected audience: got '$($Claims['aud'])', expected '$IdentifierUri'"
    exit 1
}
if ($Claims['appid'] -ne $ClientId) {
    Write-Host "[FAIL] Unexpected appid: got '$($Claims['appid'])', expected '$ClientId'"
    exit 1
}

Write-Host ""
Write-Host "[PASS] Snowflake External OAuth authentication verified"
Write-Host "       Token is valid for Snowflake SECURITY INTEGRATION with audience $($Claims['aud'])"
